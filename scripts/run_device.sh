...
DEVICES_OUTPUT=$($FLUTTER_BIN devices 2>/dev/null)
...
if [ -n "$IOS_DEVICE_ID" ] && [ -n "$ANDROID_DEVICE_ID" ]; then
    ...
    echo "🍎 iOS 빌드 시작..."
    cd "$PROJECT_DIR" && $FLUTTER_BIN run -d "$IOS_DEVICE_ID" $DART_DEFINES &
    IOS_PID=$!

    sleep 3

    echo "🤖 Android 빌드 시작..."
    cd "$PROJECT_DIR" && $FLUTTER_BIN run -d "$ANDROID_DEVICE_ID" $DART_DEFINES &
    ANDROID_PID=$!
